...
import { BookOpen, X, Loader2, AlertCircle, RefreshCw, Copy, Check } from 'lucide-react';
import { db } from '../../services/database';
import { fetchPrimarySourceText } from '../../services/geminiService';
...
export const PrimarySource: React.FC<PrimarySourceProps> = ({ source, reference, hiddenCommand, isAdmin, children }) => {
    ...
    const sourceIdBase = `${normalizeKey(source)}_${normalizeKey(reference)}`;
    ...
    const fetchSource = async (forceRegenerate = false) => {
        ...
        try {
            if (!forceRegenerate) {
                // 1. Tentar buscar no banco de dados / cache local (Universal via Supabase / IndexedDB)
                try {
            ...
            // 2. Se não existir ou forçar a regeneração, buscar via serviço resiliente do Gemini
            const text = await fetchPrimarySourceText(source, reference, hiddenCommand);
            ...
            if (text && text.trim().length > 0) {
                ...
                try {
                    ...
                    console.warn('[PrimarySource] Erro ao persistir no cache universal:', saveErr);
                }
            } else {
    ...
    return (
        <span className="relative inline-block">
            <button 
                ...
                className="text-[#D4AF37] dark:text-[#FFD700] font-bold border-b-2 border-dotted border-[#D4AF37] hover:bg-[#D4AF37]/10 transition-colors rounded px-1 italic cursor-pointer"
                        ...
                        {isLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-4 gap-3 text-center">
                                ...
                                <button 
                                    onClick={() => fetchSource(true)}
                                    disabled={isLoading}
                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[#D4AF37] dark:text-[#FFD700] bg-[#D4AF37]/10 hover:bg-[#D4AF37]/20 border border-[#D4AF37]/30 rounded-lg transition-colors cursor-pointer"
                                >
                            ...
                            <div className="prose prose-sm dark:prose-invert max-w-none font-inter text-gray-800 dark:text-gray-200 leading-relaxed">
                                ...
                                <div className="mt-4 pt-2 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-[11px] text-gray-400 italic">
                                    <button 
                                        onClick={() => {
                                            if (content) {
                                                navigator.clipboard.writeText(`${source} - ${reference}\n\n${content}`);
                                                setCopied(true);
                                                setTimeout(() => setCopied(false), 2000);
                                            }
                                        }}
                                        className="not-italic inline-flex items-center gap-1 text-[#D4AF37] hover:text-[#B89628] font-medium transition-colors cursor-pointer"
                                        title="Copiar texto da fonte"
                                    >
                                        {copied ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                                        <span>{copied ? 'Copiado!' : 'Copiar citação'}</span>
                                    </button>
                                    <span>Tradução e Contexto por Prof. Michel Felix (IA)</span>
                                </div>
                            </div>
                        )}
                    </div>
